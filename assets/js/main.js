...
window.addEventListener('DOMContentLoaded', function () {
  ...
  //  pdfFiles['http://localhost:19518/api/Workflow/Download/1062','http://localhost:19518/api/Workflow/Download/1065','http://localhost:19518/api/Workflow/Download/1066'];
    document.querySelector('#merge').addEventListener('click', () => {
        ...
        let pdfUrls = ['http://localhost:19518/api/BudgetApproval/GetUploadedPdfFileApi?instanceID=' + 14227 + '','http://localhost:19518/api/BudgetApproval/GetUploadedPdfFileApi?instanceID=' + 14483 + ''];
        //let pdfUrl = 'http://localhost:19518/api/BudgetApproval/GetUploadedPdfFileApi?instanceID=' + 14227 + ''
       // const doc1 = fetch(pdfUrl).then(res => res.arrayBuffer());
...
async function mergeAllPDFs() {
  var urls = [];
    $.ajax({
        url: 'http://localhost:19518/api/Workflow/GetFileIdAgainstInstancId/' + 14485,
        method: 'GET',
        success: function (data) {
         for (var i=0;i<data.data.length;i++){
            var id = data.data[i].id;
            urls.push('http://localhost:19518/api/Workflow/GetFileById/'+id)   
        }     
        },
        error: function (error) {
            onCommonError('File Not Found!');
            enableBtn(btn);
        },
        async:false,
    });
